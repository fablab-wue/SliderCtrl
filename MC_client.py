...
#
# MC_API surface (duck-typed): start, send/query (optional arg2/arg3), motion
# (optional extra-axis pos / home(axis)), config setters, getters
# (axis_count from CG axis, getPosition2, …), set_axis_status_callback
...
class MC_Client:
    ...
    def setLeft(self, pos=None, pos2=None, pos3=None):
        ...
        """
        if pos is None and pos2 is None and pos3 is None:
            self._cmd("SL")
            self._soft_min = self.slider_min
            if self._axis >= 2:
                self._soft_min_2 = self.slider_min_2
            ...
            if self._axis >= 3:
                if isinstance(pos3, str) and pos3.lower() == "none":
                    self._soft_min_3 = self.slider_min_3
                elif pos3 is not None and not (isinstance(pos3, str) and pos3 == "_"):
                    self._soft_min_3 = float(pos3)
        self._refresh_soft_limit_flag()

    def setRight(self, pos=None, pos2=None, pos3=None):
        """Session working-window right (`SR`). All None = bare reset to envelope.

        Extra axes: ``None`` on one axis sends skip ``_``. To clear a side, pass
